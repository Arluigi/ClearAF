...
- Record: count line, native Grid/List segmented, month eyebrows with hairline rules, 3-column 4:5 tiles with mono dates and `On device` / `Waiting to share` (ochre text) / `Shared` / `Reviewed` / `Couldn't share` in words only (no action — a 44pt hit area doesn't fit a 6pt-gutter tile); `Page 1 of 2` with outlined Previous/Next; List rows and the photo detail sheet keep full-size Share/Retry.
