...
The clinician portal opens assigned Patients and a minimal Account page. Patients supports name search and pagination, and opens dated photo review and existing routine assignment/history. Display only actual retained data; remove scores, statuses inferred from scores, fake activity, notification badges and no-op actions. Direct root/dashboard/appointments/messages/prescriptions/settings/profile routes redirect to retained screens; unlinked demo statistics and fabricated clinician credentials are not an exception. Keep authentication, verification, recovery and account-bound request guards. No new clinician provisioning or patient assignment workflow.
