...
async function debugPhotoUpload() {
  ...
  // Test 1: Check if route exists with OPTIONS
  console.log('1️⃣ Testing if upload route exists:');
  try {
    const optionsResponse = await axios.options(`${API_URL}/api/photos/upload`);
  ...
  // Test 2: Try upload without file
  ...
  // Test 3: Create a real test image and upload
