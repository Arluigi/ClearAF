import { CreateBucketCommand, PutBucketPolicyCommand, PutBucketCorsCommand } from '@aws-sdk/client-s3';
...
async function setupS3Bucket() {
  ...
  try {
    // 1. Create bucket
    console.log(`Creating bucket: ${s3Config.bucketName}`);
    try {
    ...
    // 2. Set up CORS policy (allow web app to access images)
    ...
    // 3. Set up public read policy for photos
    ...
    console.log(`\n📝 Bucket details:`);
    console.log(`   Name: ${s3Config.bucketName}`);
    console.log(`   Region: ${s3Config.region}`);
    console.log(`   Public URL: https://${s3Config.bucketName}.s3.${s3Config.region}.amazonaws.com/`);
    
    console.log(`\n🔧 Next steps:`);
    console.log(`   1. Set environment variables in Render dashboard:`);
    console.log(`      AWS_ACCESS_KEY_ID=${s3Config.accessKeyId}`);
    console.log(`      AWS_SECRET_ACCESS_KEY=***`);
    console.log(`      S3_BUCKET_NAME=${s3Config.bucketName}`);
    console.log(`      AWS_REGION=${s3Config.region}`);
