...
async function testPhotoEndpoints() {
  try {
    ...
    const patientsResponse = await fetch(`${API_URL}/users?userType=patient`, {
    ...
    for (const patient of patientsData.data) {
      ...
      // Test /api/photos/patient/{patientId}/timeline
      console.log('   Testing GET /api/photos/patient/{patientId}/timeline...');
      const timelineResponse = await fetch(`${API_URL}/photos/patient/${patient.id}/timeline?days=30`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
